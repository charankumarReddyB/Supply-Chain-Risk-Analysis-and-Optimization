...
def debug_db():
    try:
        from backend.models.database import execute_query, get_db_connection
        ...
        conn.autocommit = True # Run outside a giant transaction so we see individual successes/failures
        try:
            with conn.cursor() as cursor:
                for q in queries:
                    if q.strip():
                        ...
                        try:
                            cursor.execute(q)
                            execution_results.append({"query": first_line, "status": "SUCCESS"})
                        except Exception as query_err:
                            execution_results.append({"query": first_line, "status": "FAILED", "error": str(query_err)})
        finally:
            conn.close()
            
        # Re-check current tables after execution
        tables_after = execute_query("SELECT table_name FROM information_schema.tables WHERE table_schema='public'")
        table_names_after = [t["table_name"] for t in tables_after]
        
        return jsonify({
            "tables_before": table_names,
            "tables_after": table_names_after,
            "parsed_queries_count": len(queries),
